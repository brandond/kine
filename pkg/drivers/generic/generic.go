...
var (
	...
	FiltersRevSQL = `SELECT MAX(id) FROM kine WHERE name LIKE ? ESCAPE '^' %s GROUP BY name`
...
func Open(ctx context.Context, wg *sync.WaitGroup, driverName, dataSourceName string, connPoolConfig ConnectionPoolConfig, paramCharacter string, numbered bool, metricsRegisterer prometheus.Registerer) (*Generic, error) {
	...
	return &Generic{
		...
		GetCurrentSQL:           q(fmt.Sprintf(listSQL, "AND name >= ?"), paramCharacter, numbered) + " /* GetCurrentSQL */",
		GetCurrentValSQL:        q(fmt.Sprintf(listValSQL, "AND name >= ?"), paramCharacter, numbered) + " /* GetCurrentValSQL */",
